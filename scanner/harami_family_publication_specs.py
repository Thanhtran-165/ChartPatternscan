"""Publication-semantic specs for Harami Family chapters."""
...
HARAMI_FAMILY_PUBLICATION_SPEC_VERSION = "harami_family_publication_spec_v1"
...
PATTERN_REQUIRED_PHRASES = {
    "harami": (
        "Harami",
        "thân nến con nằm trong thân nến mẹ",
        "đóng cửa vượt đỉnh hoặc thủng đáy xác nhận",
        "mẫu hai nến nên phải đọc cùng xác nhận",
    ),
}


def build_harami_publication_spec(*, pattern_id: str, title: str, spec: Mapping[str, Any]) -> dict[str, Any]:
    publication_spec = dict(spec)
    publication_spec.update(
        {
            "status": "PASS",
            "semantic_gate_id": PUBLICATION_SEMANTIC_GATE_ID,
            "publication_spec_id": f"{pattern_id}_{HARAMI_FAMILY_PUBLICATION_SPEC_VERSION}",
            ...
            "public_story_contract": {
                "must_read_like": "public investor-facing two-candle reversal chart-pattern chapter",
