...
class RisingWedgeDetector:
    ...
    def scan_window(self, df: pd.DataFrame, pivots: Sequence[Pivot]) -> Optional[Dict[str, Any]]:
        ...
        # Sửa 14/08/2026 theo ECP ch53 Table 53.8: UD target = lowest low (đáy wedge).
        # Nếu LL đã ≥ giá breakout (hiếm) thì fallback chiều cao formation (HH−LL) trừ vào breakout.
        hh = float(df.iloc[formation_start : formation_end + 1]["high"].max())
        ll = float(df.iloc[formation_start : formation_end + 1]["low"].min())
        target_price = ll if ll < float(breakout_price) else float(breakout_price) - (hh - ll)
        quality_score = 62.0
    ...
    *,
    ...
    for window_size in (4, 5, 6):
        for i in range(len(pivots) - window_size + 1):
            ...
            record.update(_evaluate_detection(df, record, lookahead=_registry_lookahead(record.get("pattern_key") or "wedges_rising")))
            out.append(record)
...
def _add_wedge_target_calibration(stats: Dict[str, Any], scan: Mapping[str, Any], path_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    sensitivity = target_sensitivity(PatternArtifacts(PATTERN_KEY, events, path), PATTERN_KEY, horizon_days=126)
