...
def _load_ai_editorial_sections(path: Path | None) -> Dict[str, Any]:
    ...
    for section in data.get("approved_sections", []):
        if not isinstance(section, Mapping):
            continue
        section_id = str(section.get("id", ""))
        paragraphs = [str(p) for p in section.get("paragraphs", []) if str(p).strip()]
        if paragraphs:
            sections[section_id] = paragraphs
        callout = section.get("callout")
        if isinstance(callout, Mapping):
            bullets = [str(b) for b in callout.get("bullets", []) if str(b).strip()]
            if bullets:
                sections[f"{section_id}_callout"] = bullets
    mapped = {
        "summary": sections.get("intro", []) or sections.get("tom-tat", []) or sections.get("overview", []),
        "tour": sections.get("how_it_works", []) or sections.get("mau-hinh-hoat-dong", []),
        "failure": sections.get("failure", []) or sections.get("that-bai", []),
        "statistics": sections.get("statistics", []) or sections.get("thong-ke", []),
        "post_breakout": sections.get("post_breakout", []) or sections.get("hanh-vi-sau-pha-vo", []),
        "size_volume": sections.get("size_volume", []) or sections.get("kich-thuoc-va-khoi-luong", []),
        "tactics": sections.get("usage", []) or sections.get("cach-su-dung", []),
        "checklist": sections.get("checklist_callout", []) or sections.get("checklist", []) or sections.get("usage_callout", []),
    }
    return {
        "sections": {key: value for key, value in mapped.items() if value},
        "captions": data.get("example_captions", {}) if isinstance(data.get("example_captions"), Mapping) else {},
...
def build_story(payload: Mapping[str, Any], source_notes: Mapping[str, Any], events: pd.DataFrame, path_df: pd.DataFrame, charts: Mapping[str, Path], ai_sections_path: Path | None = None) -> list[Any]:
    ...
    story.append(_section_title("3", "Ví dụ trong VN100", "Một ví dụ đẹp, một ví dụ trung vị và một ví dụ thất bại"))
    examples = _select_examples(events)
    example_specs = [
